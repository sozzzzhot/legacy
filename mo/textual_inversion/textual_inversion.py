...
from mo import shared, devices, sd_hijack, processing, sd_models, images, sd_samplers, sd_hijack_checkpoint, errors
import mo.textual_inversion.dataset
from mo.textual_inversion.learn_schedule import LearnRateScheduler

from mo.textual_inversion.image_embedding import embedding_to_b64, embedding_from_b64, insert_image_data_embed, extract_image_data_embed, caption_image_overlay
from mo.textual_inversion.logging import save_settings_to_file
...
class EmbeddingDatabase:
    ...
    def load_textual_inversion_embeddings(self, force_reload=False):
        ...
        if self.previously_displayed_embeddings != displayed_embeddings:
            ...
            print(f"Textual inversion beddings loaded({len(self.word_embeddings)}): {', '.join(self.word_embeddings.keys())}")
            if self.skipped_embeddings:
                print(f"Textual inversion beddings skipped({len(self.skipped_embeddings)}): {', '.join(self.skipped_embeddings.keys())}")
...
def train_embedding(id_task, embedding_name, learn_rate, batch_size, gradient_step, data_root, log_directory, training_width, training_height, varsize, steps, clip_grad_mode, clip_grad_value, shuffle_tags, tag_drop_out, latent_sampling_method, use_weight, create_image_every, save_embedding_every, template_filename, save_image_with_stored_embedding, preview_from_txt2img, preview_prompt, preview_negative_prompt, preview_steps, preview_sampler_index, preview_cfg_scale, preview_seed, preview_width, preview_height):
    ...
    if save_embedding_every > 0:
        embedding_dir = os.path.join(log_directory, "beddings")
    ...
    dl = mo.textual_inversion.dataset.PersonalizedDataLoader(ds, latent_sampling_method=latent_sampling_method, batch_size=ds.batch_size, pin_memory=pin_memory)
