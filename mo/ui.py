...
from mo.call_queue import wrap_gradio_gpu_call, wrap_queued_call, wrap_gradio_call

from mo import sd_hijack, sd_models, script_callbacks, ui_extensions, deepbooru, sd_vae, extra_networks, ui_common, ui_postprocessing, progress, ui_loadsave, errors, shared_items, ui_settings, timer, sysinfo
from mo.ui_components import FormRow, FormGroup, ToolButton, FormHTML
from mo.paths import script_path
from mo.ui_common import create_refresh_button
from mo.ui_gradio_extensions import reload_javascript


from mo.shared import opts, cmd_opts

import mo.codeformer_model
import mo.generation_parameters_copypaste as parameters_copypaste
import mo.gfpgan_model
import mo.hypernetworks.ui
import mo.scripts
import mo.shared as shared
import mo.styles
import mo.textual_inversion.ui
from mo import prompt_parser
from mo.sd_hijack import model_hijack
from mo.sd_samplers import samplers, samplers_for_img2img
from mo.textual_inversion import textual_inversion
import mo.hypernetworks.ui
from mo.generation_parameters_copypaste import image_from_url_text
import mo.extras
...
def add_style(name: str, prompt: str, negative_prompt: str):
    ...
    style = mo.styles.PromptStyle(name, prompt, negative_prompt)
...
def calc_resolution_hires(enable, width, height, hr_scale, hr_resize_x, hr_resize_y):
    from mo import processing, devices
...
def create_ui():
    ...
    mo.scripts.scripts_current = mo.scripts.scripts_txt2img
    mo.scripts.scripts_txt2img.initialize_scripts(is_img2img=False)
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with FormRow(variant='compact', elem_id="txt2img_extra_networks", visible=False) as extra_networks:
            from mo import ui_extra_networks
            extra_networks_ui = ui_extra_networks.create_ui(extra_networks, extra_networks_button, 'txt2img')

        with gr.Row().style(equal_height=False):
            with gr.Column(variant='compact', elem_id="txt2img_settings"):
                mo.scripts.scripts_txt2img.prepare_ui()
                ...
                for category in ordered_ui_categories():
                    if category == "sampler":
                        ...
                        with FormGroup(elem_id="txt2img_script_container"):
                            custom_inputs = mo.scripts.scripts_txt2img.setup_ui()

                    else:
                        mo.scripts.scripts_txt2img.setup_ui_for_section(category)
            ...
            txt2img_args = dict(
                fn=wrap_gradio_gpu_call(mo.txt2img.txt2img, extra_outputs=[None, '', '']),
                _js="submit",
                inputs=[
                    dummy_component,
                    txt2img_prompt,
            ...
            txt2img_paste_fields = [
                ...
                *mo.scripts.scripts_txt2img.infotext_fields
            ]
    ...
    mo.scripts.scripts_current = mo.scripts.scripts_img2img
    mo.scripts.scripts_img2img.initialize_scripts(is_img2img=True)
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with FormRow().style(equal_height=False):
            with gr.Column(variant='compact', elem_id="img2img_settings"):
                ...
                mo.scripts.scripts_img2img.prepare_ui()
                ...
                for category in ordered_ui_categories():
                    if category == "sampler":
                        ...
                        with FormGroup(elem_id="img2img_script_container"):
                            custom_inputs = mo.scripts.scripts_img2img.setup_ui()
                        ...
                        mo.scripts.scripts_img2img.setup_ui_for_section(category)
            ...
            img2img_args = dict(
                fn=wrap_gradio_gpu_call(mo.img2img.img2img, extra_outputs=[None, '', '']),
                _js="submit_img2img",
                inputs=[
                    dummy_component,
                    dummy_component,
            ...
            img2img_paste_fields = [
                ...
                (denoising_strength, "Denoising strength"),
                (mask_blur, "Mask blur"),
                *mo.scripts.scripts_img2img.infotext_fields
            ]
    ...
    mo.scripts.scripts_current = None
    ...
    with gr.Blocks(analytics_enabled=False) as pnginfo_interface:
        ...
        image.change(
            fn=wrap_gradio_call(mo.extras.run_pnginfo),
            inputs=[image],
        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant='compact'):
                ...
                with FormRow(elem_id="modelmerger_models"):
                    primary_model_name = gr.Dropdown(mo.sd_models.checkpoint_tiles(), elem_id="modelmerger_primary_model_name", label="Primary model (A)")
                    create_refresh_button(primary_model_name, mo.sd_models.list_models, lambda: {"choices": mo.sd_models.checkpoint_tiles()}, "refresh_checkpoint_A")

                    secondary_model_name = gr.Dropdown(mo.sd_models.checkpoint_tiles(), elem_id="modelmerger_secondary_model_name", label="Secondary model (B)")
                    create_refresh_button(secondary_model_name, mo.sd_models.list_models, lambda: {"choices": mo.sd_models.checkpoint_tiles()}, "refresh_checkpoint_B")

                    tertiary_model_name = gr.Dropdown(mo.sd_models.checkpoint_tiles(), elem_id="modelmerger_tertiary_model_name", label="Tertiary model (C)")
                    create_refresh_button(tertiary_model_name, mo.sd_models.list_models, lambda: {"choices": mo.sd_models.checkpoint_tiles()}, "refresh_checkpoint_C")
    ...
    with gr.Blocks(analytics_enabled=False) as train_interface:
        ...
        with gr.Row(variant="compact").style(equal_height=False):
            with gr.Tabs(elem_id="train_tabs"):
                ...
                with gr.Tab(label="Create hypernetwork", id="create_hypernetwork"):
                    ...
                    new_hypernetwork_activation_func = gr.Dropdown(value="linear", label="Select activation function of hypernetwork. Recommended : Swish / Linear(none)", choices=mo.hypernetworks.ui.keys, elem_id="train_new_hypernetwork_activation_func")
                ...
                with gr.Tab(label="Train", id="train"):
                    ...
                    with FormRow():
                        template_file = gr.Dropdown(label='Prompt template', value="none.txt", elem_id="train_template_file", choices=get_textual_inversion_template_names())
        ...
        create_embedding.click(
            fn=mo.textual_inversion.ui.create_embedding,
            inputs=[
                new_embedding_name,
                initialization_text,
                nvpt,
        ...
        create_hypernetwork.click(
            fn=mo.hypernetworks.ui.create_hypernetwork,
            inputs=[
        ...
        run_preprocess.click(
            fn=wrap_gradio_gpu_call(mo.textual_inversion.ui.preprocess, extra_outputs=[gr.update()]),
        ...
        train_embedding.click(
            fn=wrap_gradio_gpu_call(mo.textual_inversion.ui.train_embedding, extra_outputs=[gr.update()]),
            _js="start_training_textual_inversion",
            inputs=[
                dummy_component,
                train_embedding_name,
        ...
        train_hypernetwork.click(
            fn=wrap_gradio_gpu_call(mo.hypernetworks.ui.train_hypernetwork, extra_outputs=[gr.update()]),
            _js="start_training_textual_inversion",
            inputs=[
                dummy_component,
                train_hypernetwork_name,
    ...
    with gr.Blocks(theme=shared.gradio_theme, analytics_enabled=False, title="Stable Diffusion") as demo:
        ...
        def modelmerger(*args):
            try:
                results = mo.extras.run_modelmerger(*args)
            except Exception as e:
                errors.report("Error loading/saving model file", exc_info=True)
                mo.sd_models.list_models()  # to remove the potentially missing models from the list
                return [*[gr.Dropdown.update(choices=mo.sd_models.checkpoint_tiles()) for _ in range(4)], f"Error merging checkpoints: {e}"]
            return results
