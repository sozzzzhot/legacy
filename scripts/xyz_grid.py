...
import mo.scripts as scripts
import gradio as gr

from mo import images, sd_samplers, processing, sd_models, sd_vae, sd_samplers_kdiffusion
from mo.processing import process_images, Processed, StableDiffusionProcessingTxt2Img
from mo.shared import opts, state
...
from mo.ui_components import ToolButton
...
def apply_checkpoint(p, x, xs):
    info = mo.sd_models.get_closet_checkpoint_match(x)
    if info is None:
...
def confirm_checkpoints(p, xs):
    for x in xs:
        if mo.sd_models.get_closet_checkpoint_match(x) is None:
            raise RuntimeError(f"Unknown checkpoint: {x}")
...
def find_vae(name: str):
    if name.lower() in ['auto', 'automatic']:
        return mo.sd_vae.unspecified
    if name.lower() == 'none':
        return None
    else:
        choices = [x for x in sorted(mo.sd_vae.vae_dict, key=lambda x: len(x)) if name.lower().strip() in x.lower()]
        if len(choices) == 0:
            print(f"No VAE found for {name}; using automatic")
            return mo.sd_vae.unspecified
        else:
            return mo.sd_vae.vae_dict[choices[0]]


def apply_vae(p, x, xs):
    mo.sd_vae.reload_vae_weights(shared.sd_model, vae_file=find_vae(x))
...
class SharedSettingsStackHelper(object):
    ...
    def __exit__(self, exc_type, exc_value, tb):
        ...
        mo.sd_models.reload_model_weights()
        mo.sd_vae.reload_vae_weights()
